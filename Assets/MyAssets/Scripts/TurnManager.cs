...
public class TurnManager : MonoBehaviour
{
    ...
    public static void EndTurn(){//Se llama con cada pass
        ...
        if(countPass>1){//A las dos veces que se acaba el turno sin jugar cartas se avanza de ronda
            NextRound();
            Debug.Log("Se acaba de empezar otra ronda. Le toca a: "+PlayerTurn);
        }else{
            if(PlayerTurn==1){//Se cambia de turno cada vez que se acabe el turno y no se acabe la ronda
               PlayerTurn=2;
                CardsPlayed=0;
            }else{
                PlayerTurn=1;
    ...
    Debug.Log("Iniciando nueva ronda en turno de: P"+PlayerTurn);
        ...
        Debug.Log("P1>P2");
            if(PlayerTurn==2){//P1 comienza el proximo turno
                Debug.Log("Player Turn(2): "+PlayerTurn);
                PlayerTurn=1;//En este caso solo hay que arreglar a quien le toca el turno porque el campo se cambia ya de por si
            }else{
                Debug.Log("Player Turn(1): "+PlayerTurn);
        ...
        Debug.Log("P2>P1");
            if(PlayerTurn==1){//P2 comienza el proximo turno
                Debug.Log("Player Turn(1): "+PlayerTurn);
                PlayerTurn=2;//En este caso solo hay que arreglar a quien le toca el turno porque el campo se cambia ya de por si
            }else{
                Debug.Log("Player Turn(2): "+PlayerTurn);
                ClickPassB();//En este caso hay que clickear el passbutton porque esto deshace el cambio de campo
            }
    ...
    //Cuando un jugador que deberia comenzar la ronda gana en su turno el boton hace que parezca que el que debe jugar es el otro
