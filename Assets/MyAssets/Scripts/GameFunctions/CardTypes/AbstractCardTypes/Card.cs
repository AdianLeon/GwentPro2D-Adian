...
public abstract class Card : MonoBehaviour, IGlow, IShowZone, IPointerEnterHandler, IPointerExitHandler
{
    ...
    public virtual void ShowZone()
    {
        ...
        FindObjectsOfType<DropZone>().Where(zone => zone.IsDropValid(gameObject.GetComponent<DraggableCard>())).ForEach(zone => zone.OnGlow());
    }
    ...
    {//Si el jugador puede jugar, tiene componente DraggableCard, no se esta arrastrando ninguna carta y ademas esta en la mano se iluminan las zonas donde la carta se puede soltar
        ...
        if (gameObject.GetComponent<DraggableCard>() == null) { return; }//Si no tiene componente DraggableCard
        if (DraggableCard.IsOnDrag) { return; }//Si se esta arrastrando alguna carta
        if (!gameObject.GetComponent<DraggableCard>().IsOnHand) { return; }//Si no esta en la mano
        ShowZone();
    }
