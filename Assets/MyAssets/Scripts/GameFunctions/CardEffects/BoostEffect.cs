...
public class BoostEffect : CardEffect
{
    override public void TriggerEffect(){
        ...
        for(int i=0;i<target.transform.childCount;i++){
            if(target.transform.GetChild(i).GetComponent<UnitCard>()!=null && target.transform.GetChild(i).GetComponent<UnitCard>().whichQuality==UnitCard.quality.Gold){//Si es de oro
