...
public static class CustomLinqExtensions
{
    ...
    public static IEnumerable<T> Randomize<T>(this IEnumerable<T> items)
    {
        ...
        for (int i = 0; i < itemsList.Length; i++) { int randomPos = UnityEngine.Random.Range(0, itemsList.Length); (itemsList[i], itemsList[randomPos]) = (itemsList[randomPos], itemsList[i]); }
        return itemsList;
    }
...
public static class CustomGameObjectExtensions
{
    ...
    {//Devuelve los descendientes carta de un GameObject
        ...
        foreach (Transform card in place.transform) { cards.AddRange(card.gameObject.CardsInside<T>()); }//Anade a su lista lo que sus hijos devuelvan
